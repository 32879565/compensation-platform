...
门店及厅面/厨房负责人的正式来源是钉钉组织通讯录，数据流为“钉钉 API → 同步预览 → 人事确认
→ 本系统”，**不需要也不应先导出钉钉 Excel**。该操作只供集团 HR 使用：账号必须具备全局
...
2. 将 `COMP_DINGTALK_STORE_ROOT_NAMES` 配置为钉钉中承载在营门店的根部门**精确名称**，多个名称
   用英文逗号分隔。系统只在这些根部门的后代中识别名称以“店”结尾或与本地门店同名的部门；
   根部门改名或新增经营板块时，必须先同步更新该变量并重新预览，不能通过 Excel 补录绕过。
...
1. 在“组织架构”页点击“同步钉钉门店与负责人”。系统直接读取钉钉部门、成员、职位和部门归属，
...
| 动作 | 运维含义 |
| --- | --- |
| `LINK` | 将唯一匹配的已有门店关联到稳定的钉钉部门 ID。 |
| `CREATE` | 在已解析的本地上级组织下新建在营门店；应用后必须人工补齐城市。 |
| `ACTIVATE` | 将匹配的历史门店恢复为在营并关联钉钉部门 ID。 |
| `UPDATE` | 按钉钉更新已关联门店名称并保持在营状态。 |
| `MISSING_IN_DINGTALK` | 本地在营门店未出现在本次钉钉范围中；作为覆盖冲突保留，不自动删除门店。应先检查根部门配置、钉钉归属及是否停业。 |

负责人动作含义如下：默认钉钉职位“店长”对应厅面、“厨房经理”对应厨房；正式值以上述配置为准。

| 动作 | 运维含义 |
| --- | --- |
| `ASSIGN` | 以稳定钉钉身份或唯一工号匹配员工，替换该“门店 + 部门”的复核人；旧负责人的对应范围随之撤销。 |
| `REMOVE` | 钉钉中已没有该部门的有效唯一负责人，或本地门店不再被钉钉范围覆盖；确认后明确撤销现有复核范围。必须逐项人工确认。 |
| `CONFLICT` | 存在多人负责人、弱姓名匹配、身份/账号冲突、同一负责人跨多店等不安全情况；任何负责人冲突都会阻止整个批次应用。 |
