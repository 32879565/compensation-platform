...
- **任务**：`dingtalk` 客户端（token 缓存、工作通知 API）、部门→收件人路由服务、推送记录表、`payroll_run` 确认钩子、前端门店负责人配置页 + 推送记录/重推。
- **验证**：`pytest tests/dingtalk`（路由按门店+部门隔离、收件人解析、失败重试、审计、沙盒模式不真发）；用测试凭据在沙盒验证。
- **退出标准**：厅面→店长/厨房→厨房经理路由正确且隔离；沙盒清单可核对；审计完整。
- **回滚**：`alembic downgrade`（部门列/推送表/负责人配置）+ 弃分支；关特性开关停推送。
- **开放问题**：Q11 一店多店长/无厨房经理时的兜底？Q12 部门=其他(如后勤/店长本人)的薪酬推给谁？Q13 推送频率与免打扰时段？
