...
#### S13 · 薪资计算批次 · 核算 · 复核 · 锁定（详见 [docs/payroll-batch-spec.md](../docs/payroll-batch-spec.md)）
- **上下文**：业务方 2026-07-19 给出完整规格。把 S11 引擎+S12 社保个税串成一个**月度薪资批次**，经门店/部门复核、异议处理、人事锁定与版本保留。**原 S13b 钉钉推送/申诉并入此步**——门店确认/异议即复核流程，钉钉是分发+通知+发起异议的通道。
- **前置（补数据模型）**：`employee` 加 部门(厅面/厨房)+职位/特殊职位标记；`attendance` 记录出勤工时+应出勤天数可人事调整(带原因)；`salary_component_def` 区分 固定/浮动补贴、房补。
- **引擎 v2（替换 S11 v1 占位公式，以规格为准）**：
  - 出勤工资 = 综合薪资 ÷ 当月应出勤天数 × 实际计薪出勤天数（实际不超当月天数，2 位小数四舍五入）。
  - 实际出勤天数：厅面=出勤工时÷9，厨房=出勤工时÷9.5，特殊职位按审批实际天数。
  - 法定节假日：有出勤 3000/应出勤×3，无出勤 ×1；入职晚于法定日当月不享。⚠️3000 基数待确认。
  - 房补：固定全额；15 天规则；入离职当月按比例。补贴分固定/浮动，人工录入必须填原因依据。
  - 押金：新员工当月扣 600；不够扣则当月不发、押金不扣、结转下月。
  - 应发 = 出勤工资+加班+法定节假日+固定补贴+浮动补贴+房补+上月补发−上月补扣。
- **批次与状态机**：`payroll_batch`(薪资月份/考勤起止/核算状态/门店确认/人事审核/锁定状态)，状态 待门店确认→存在异议→待人事处理→已确认→已锁定。并发互斥：同周期同批次单例。
- **复核流程**：按 门店×部门 分发，负责人只见本范围；确认无误→部门已确认；提交异议→选具体员工+工资项+意见。
- **异议处理**：人事 同意/驳回/要求补充材料；**同意后改源基础数据并自动重算所有关联项**（出勤工资/按出勤的补贴/房补/应发/实发），禁止只改最终金额。
- **修改记录**：前值/后值/项目/原因/申请人/审批人/时间/附件/重算结果，全审计。
- **锁定/解锁**：全部确认+人事审核后锁定，锁后禁改；解锁须人事填原因，**保留新旧版本不覆盖历史**。
- **工资条**：员工自助查本人逐项明细，限本人。
- **验证**：`pytest tests/payroll_batch`（批次状态机、按门店×部门分发与范围隔离、异议选项目、同意后自动重算联动、锁定禁改、解锁版本保留、押金结转、法定节假日各分支、出勤工资公式、审计）；E2E 一期从核算到锁定。
- **退出标准**：不变量 5/11 成立；批次可核算/复核/异议/锁定；改源数据即联动重算；版本不覆盖。
- **回滚**：`alembic downgrade` + 弃分支；作废未锁定批次。
- **开放问题**：见 spec 中 ⚠️（出勤日最低工时/小数天、房补 15 天精确口径、法定节假日 3000 基数）+ 原 Q11–Q16。

#### S13b · 钉钉薪酬推送（**已并入 S13 复核流程**，此处保留钉钉通道细节）
> 2026-07-19 业务规格把「门店确认/异议」正式定义为 S13 复核流程；钉钉推送=分发+通知+发起异议的通道。以下钉钉客户端/路由/申诉细节仍在 S13 内实现。
