...
| Q17 | **计薪口径（S11 引擎 v1 默认，待业务确认）**：①出勤折算除数=应出勤天数(cap 1.0)；②加班倍数统一 1.5，未区分休息日 2倍/法定 3倍；③加班基数=BASE+POSITION 满月额/21.75/8；④兼职工时=实出勤天数×8；⑤补贴全额不折算；⑥试用期系数默认 1(no-op)，试用薪资靠结构生效日期化体现。以上均可调，业务确认后改 RuleConfig/service。 | S11 | 见上，v1 占位 |
