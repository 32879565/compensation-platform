...
def upgrade() -> None:
    ...
    op.create_index(op.f('ix_import_staging_row_batch_id'), 'import_staging_row', ['batch_id'], unique=False)
    op.create_table(
        'salary_record',
        sa.Column('period', sa.String(length=7), nullable=False),
        sa.Column('emp_no', sa.String(length=32), nullable=True),
    ...
    for col in ('emp_no', 'employee_id', 'import_batch_id', 'name', 'org_unit_id', 'period', 'source', 'store_name'):
        op.create_index(op.f(f'ix_salary_record_{col}'), 'salary_record', [col], unique=False)


def downgrade() -> None:
    for col in ('store_name', 'source', 'period', 'org_unit_id', 'name', 'import_batch_id', 'employee_id', 'emp_no'):
