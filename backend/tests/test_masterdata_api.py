...
def test_emp_no_conflict_returns_409(client, db_session):
    ...
    _user(db_session, "hr", ["GROUP_HR"])
    h = _token(client, "hr")
    payload = {"emp_no": "E001", "name": "张三", "org_unit_id": orgs["gz_store"].id}
...
def test_region_manager_cannot_see_other_region_employee(client, db_session):
    ...
    client.post(
        "/api/employees",
        headers=hr,
        json={"emp_no": "SZ1", "name": "李四", "org_unit_id": orgs["sz_store"].id},
    )
...
def test_scoped_writer_cannot_create_in_unseen_org(client, db_session):
    ...
    ok = client.post(
        "/api/employees",
        headers=h,
        json={"emp_no": "GZ1", "name": "赵六", "org_unit_id": orgs["gz_store"].id},
    )
    ...
    blocked = client.post(
        "/api/employees",
        headers=h,
        json={"emp_no": "SZ2", "name": "王五", "org_unit_id": orgs["sz_store"].id},
    )
...
def test_global_role_not_restricted_by_scope(client, db_session):
    ...
    r = client.post(
        "/api/employees",
        headers=hr,
        json={"emp_no": "SZ3", "name": "王五", "org_unit_id": orgs["sz_store"].id},
    )
...
def _create_emp(client, headers, org_id, emp_no="E001", **extra):
    return client.post(
        "/api/employees",
        headers=headers,
        json={"emp_no": emp_no, "name": "张三", "org_unit_id": org_id, **extra},
    )
