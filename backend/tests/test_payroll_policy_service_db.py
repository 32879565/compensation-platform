...
def test_build_inputs_loads_effective_policy_deduction_flags_and_locked_ytd(db_session) -> None:
    ...
    db_session.add_all([employee, component, schedule, HolidayCalendarPeriod(period="2026-07", is_finalized=True)])
    db_session.flush()
    ...
    _locked_result(
        ...
        snapshot=_tax_snapshot(
            taxable="9000", contribution="400", special="100", withheld="100", employment_months=1
        ),
    )
    _locked_result(
        db_session,
        ...
        version=2,
        snapshot=_tax_snapshot(
            taxable="10000", contribution="500", special="200", withheld="150", employment_months=1
        ),
    )
    _locked_result(
        db_session,
        ...
        version=1,
        snapshot=_tax_snapshot(
            taxable="11000", contribution="600", special="300", withheld="200", employment_months=2
        ),
    )
    db_session.flush()
