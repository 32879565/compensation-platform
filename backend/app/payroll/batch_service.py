...
_SUPPORTED_RECOMPUTE_RULE_VERSIONS = frozenset({"v2", "v3"})
...
def _payroll_tax_from_snapshot(
    ...
    try:
        ...
        policy = PayrollPolicyContext(
            ...
            tax_policy=TaxPolicyInput(
                monthly_basic_deduction=_snapshot_decimal(
                    value["monthly_basic_deduction"], "policy.monthly_basic_deduction"
                ),
                brackets=tuple(
                    TaxBracket(
                        upper_bound=(
        ...
        if len(policy.social_policy.rules) != len(social_rules) or len(
            policy.tax_policy.brackets
        ) != len(tax_brackets):
            raise BatchError("The persisted input snapshot has an invalid payroll tax entry.")
...
def _input_from_snapshot(
    prior_result: PayrollResult, attendance_changes: Mapping[str, Decimal]
    ...
    snapshot = prior_result.input_snapshot
    ...
    try:
        ...
        inp = EmployeeInput(
            ...
            expected_days_rule_id=(
                int(snapshot["expected_days_rule_id"])
        ...
        missing_component_ids = [
            int(component_id) for component_id in snapshot["missing_component_ids"]
        ]
    except (KeyError, TypeError, ValueError) as exc:
...
def _recompute_result_from_snapshot(
    ...
    inp, missing_component_ids = _input_from_snapshot(prior_result, attendance_changes)
...
def approve_batch(session: Session, batch: PayrollBatch, user_id: int) -> None:
    ...
    # ``user_id`` is carried by the router audit record; the current schema has
    # no separate final-approval actor column.
    _ = user_id
...
def raise_dispute(
    ...
    if salary_item not in _ATTENDANCE_DISPUTE_ITEM_CODES:
        raise BatchError("This salary item has no supported source-data correction workflow.")
    dispute = CompDispute(
        batch_id=batch.id,
        batch_version=batch.version,
    ...
    *,
    ...
    att = session.scalars(
        select(AttendanceRecord).where(
            AttendanceRecord.employee_id == emp.id, AttendanceRecord.period == batch.period
        )
    ).first()
    if att is None:
    ...
    before = _attendance_values(att)
    for field, value in validated_changes.items():
        setattr(att, field, value)
    session.flush()
    after = _attendance_values(att)

    new_result = recompute_employee(
        session,
        batch,
        emp,
        attendance_changes=validated_changes,
    )
    session.add(
        AdjustmentRecord(
            batch_id=batch.id,
            ...
            recompute_result={
                ...
                "batch_version": new_result.batch_version,
                "rule_version": new_result.rule_version,
                "input_snapshot": new_result.input_snapshot,
                "gross": str(new_result.gross),
                "net": str(new_result.net),
            },
        )
    )
